...
class RabbitMQAdmin(CommandParser):
    ...
    def verifyResult(self, cmd, result):
        ...
        match = re.search(r'rabbitmqadmin \d.\d', rabbitmqversion_result, re.MULTILINE)
        if match:
            clear = True

        match = re.search( r'No such file or directory', cmd.result.output, re.MULTILINE)
        if match:
            ...
            log.debug("rabbitmqadmin command not found")
            clear = False
        elif cmd.result.exitCode != 0:
            summary = 'rabbitmqctladmin had non-zero exit code - see event message'
            message = 'Please refer to http://wiki.zenoss.org/ZenPack:RabbitMQ for setup instructions.'
            clear = False
        else:
            log.debug('Unable to gather with rabbitmqadmin, unknown error')
